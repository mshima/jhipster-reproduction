...
import com.okta.developer.notification.security.SecurityUtils;
...
public class UserService {
    ...
    public Flux<AdminUserRest> getAllManagedUsers(Pageable pageable) {
        return userRepository.findAllWithAuthorities(pageable).map(AdminUserRest::new);
    }

    @Transactional(readOnly = true)
    public Flux<UserRest> getAllPublicUsers(Pageable pageable) {
        return userRepository.findAllByIdNotNullAndActivatedIsTrue(pageable).map(UserRest::new);
    }
     ...
     */
    @Transactional
    public Mono<AdminUserRest> getUserFromAuthentication(AbstractAuthenticationToken authToken) {
        ...
        return syncUserWithIdP(attributes, user).flatMap(u -> Mono.just(new AdminUserRest(u)));
    }
