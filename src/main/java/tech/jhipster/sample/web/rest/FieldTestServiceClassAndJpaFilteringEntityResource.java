 ...
 */
@RestController
@RequestMapping("/api/field-test-service-class-and-jpa-filtering-entities")
public class FieldTestServiceClassAndJpaFilteringEntityResource {

    private static final Logger LOG = LoggerFactory.getLogger(FieldTestServiceClassAndJpaFilteringEntityResource.class);
    ...
    public Mono<ResponseEntity<FieldTestServiceClassAndJpaFilteringEntity>> createFieldTestServiceClassAndJpaFilteringEntity(
        ...
        return fieldTestServiceClassAndJpaFilteringEntityService.save(fieldTestServiceClassAndJpaFilteringEntity).map(result -> {
            try {
                return ResponseEntity.created(new URI("/api/field-test-service-class-and-jpa-filtering-entities/" + result.getId()))
    ...
    public Mono<ResponseEntity<FieldTestServiceClassAndJpaFilteringEntity>> updateFieldTestServiceClassAndJpaFilteringEntity(
        ...
        return fieldTestServiceClassAndJpaFilteringEntityRepository.existsById(id).flatMap(exists -> {
            if (!exists) {
                return Mono.error(new BadRequestAlertException("Entity not found", ENTITY_NAME, "idnotfound"));
            }

            return fieldTestServiceClassAndJpaFilteringEntityService
                .update(fieldTestServiceClassAndJpaFilteringEntity)
    ...
    /**
     * {@code GET  /field-test-service-class-and-jpa-filtering-entities} : get all the Field Test Service Class And Jpa Filtering Entities.
     *
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of Field Test Service Class And Jpa Filtering Entities in body.
     */
    @GetMapping(value = "", produces = MediaType.APPLICATION_JSON_VALUE)
    public Flux<FieldTestServiceClassAndJpaFilteringEntity> getAllFieldTestServiceClassAndJpaFilteringEntities(
        FieldTestServiceClassAndJpaFilteringEntityCriteria criteria
    ) {
        LOG.debug("REST request to get FieldTestServiceClassAndJpaFilteringEntities by criteria: {}", criteria);
        return fieldTestServiceClassAndJpaFilteringEntityService.findByCriteria(criteria);
    }

    /**
     * {@code GET  /field-test-service-class-and-jpa-filtering-entities/count} : count all the fieldTestServiceClassAndJpaFilteringEntities.
     *
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the count in body.
     */
    @GetMapping("/count")
    public Mono<ResponseEntity<Long>> countFieldTestServiceClassAndJpaFilteringEntities(
        FieldTestServiceClassAndJpaFilteringEntityCriteria criteria
    ) {
        LOG.debug("REST request to count FieldTestServiceClassAndJpaFilteringEntities by criteria: {}", criteria);
        return fieldTestServiceClassAndJpaFilteringEntityService
    ...
    public Mono<ResponseEntity<FieldTestServiceClassAndJpaFilteringEntity>> getFieldTestServiceClassAndJpaFilteringEntity(
        ...
        Mono<FieldTestServiceClassAndJpaFilteringEntity> fieldTestServiceClassAndJpaFilteringEntity =
            fieldTestServiceClassAndJpaFilteringEntityService.findOne(id);
     ...
     */
    @DeleteMapping("/{id}")
    public Mono<ResponseEntity<Void>> deleteFieldTestServiceClassAndJpaFilteringEntity(@PathVariable("id") Long id) {
