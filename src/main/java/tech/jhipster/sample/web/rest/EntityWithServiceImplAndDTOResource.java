 ...
 */
@RestController
@RequestMapping("/api/entity-with-service-impl-and-dtos")
public class EntityWithServiceImplAndDTOResource {

    private static final Logger LOG = LoggerFactory.getLogger(EntityWithServiceImplAndDTOResource.class);
    ...
    public Mono<ResponseEntity<EntityWithServiceImplAndDTODTO>> createEntityWithServiceImplAndDTO(
        ...
        return entityWithServiceImplAndDTOService.save(entityWithServiceImplAndDTODTO).map(result -> {
            try {
    ...
    public Mono<ResponseEntity<EntityWithServiceImplAndDTODTO>> updateEntityWithServiceImplAndDTO(
        ...
        return entityWithServiceImplAndDTORepository.existsById(id).flatMap(exists -> {
            if (!exists) {
                return Mono.error(new BadRequestAlertException("Entity not found", ENTITY_NAME, "idnotfound"));
            }

            return entityWithServiceImplAndDTOService
                .update(entityWithServiceImplAndDTODTO)
    ...
    public Mono<ResponseEntity<EntityWithServiceImplAndDTODTO>> partialUpdateEntityWithServiceImplAndDTO(
        ...
        return entityWithServiceImplAndDTORepository.existsById(id).flatMap(exists -> {
            ...
            Mono<EntityWithServiceImplAndDTODTO> result = entityWithServiceImplAndDTOService.partialUpdate(entityWithServiceImplAndDTODTO);
    ...
    /**
     * {@code GET  /entity-with-service-impl-and-dtos} : get all the Entity With Service Impl And DTOS.
     *
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of Entity With Service Impl And DTOS in body.
     */
    ...
    /**
     * {@code GET  /entity-with-service-impl-and-dtos} : get all the Entity With Service Impl And DTOS as a stream.
     * @return the {@link Flux} of Entity With Service Impl And DTOS.
     */
     ...
     */
    @GetMapping("/{id}")
    public Mono<ResponseEntity<EntityWithServiceImplAndDTODTO>> getEntityWithServiceImplAndDTO(@PathVariable("id") Long id) {
     ...
     */
    @DeleteMapping("/{id}")
    public Mono<ResponseEntity<Void>> deleteEntityWithServiceImplAndDTO(@PathVariable("id") Long id) {
