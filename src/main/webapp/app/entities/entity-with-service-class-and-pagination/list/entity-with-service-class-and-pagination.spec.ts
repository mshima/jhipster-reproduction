...
import { sampleWithRequiredData } from '../entity-with-service-class-and-pagination.test-samples';
...
describe('EntityWithServiceClassAndPagination Management Component', () => {
  ...
  let comp: EntityWithServiceClassAndPagination;
  let fixture: ComponentFixture<EntityWithServiceClassAndPagination>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(EntityWithServiceClassAndPagination);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 11699 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.entityWithServiceClassAndPaginations()[0]).toEqual(expect.objectContaining({ id: 11699 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 11699 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.entityWithServiceClassAndPaginations()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 11699 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to entityWithServiceClassAndPaginationService', () => {
      const entity = { id: 11699 };
      vi.spyOn(service, 'getEntityWithServiceClassAndPaginationIdentifier');
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.entityWithServiceClassAndPaginationsParams()).toMatchObject({ sort: ['id,desc'] });
  });
