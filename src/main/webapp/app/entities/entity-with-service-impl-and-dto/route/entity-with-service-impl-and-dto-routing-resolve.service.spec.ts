...
import entityWithServiceImplAndDTOResolve from './entity-with-service-impl-and-dto-routing-resolve.service';
...
describe('EntityWithServiceImplAndDTO routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return IEntityWithServiceImplAndDTO returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          entityWithServiceImplAndDTOResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(entityWithServiceImplAndDTOResolve(mockActivatedRouteSnapshot))).rejects.toThrow(
