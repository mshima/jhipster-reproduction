...
import { sampleWithRequiredData } from '../entity-with-service-impl-and-dto.test-samples';
...
describe('EntityWithServiceImplAndDTO Management Component', () => {
  ...
  let comp: EntityWithServiceImplAndDTO;
  let fixture: ComponentFixture<EntityWithServiceImplAndDTO>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(EntityWithServiceImplAndDTO);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 30188 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.entityWithServiceImplAndDTOS()[0]).toEqual(expect.objectContaining({ id: 30188 }));
  });

  describe('trackId', () => {
    it('should forward to entityWithServiceImplAndDTOService', () => {
      const entity = { id: 30188 };
      vi.spyOn(service, 'getEntityWithServiceImplAndDTOIdentifier');
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.entityWithServiceImplAndDTOSParams()).toMatchObject({ sort: ['id,desc'] });
  });
