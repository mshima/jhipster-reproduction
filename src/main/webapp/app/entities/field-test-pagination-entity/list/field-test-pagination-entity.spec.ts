...
import { sampleWithRequiredData } from '../field-test-pagination-entity.test-samples';
...
describe('FieldTestPaginationEntity Management Component', () => {
  ...
  let comp: FieldTestPaginationEntity;
  let fixture: ComponentFixture<FieldTestPaginationEntity>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(FieldTestPaginationEntity);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 16266 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.fieldTestPaginationEntities()[0]).toEqual(expect.objectContaining({ id: 16266 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 16266 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.fieldTestPaginationEntities()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 16266 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to fieldTestPaginationEntityService', () => {
      const entity = { id: 16266 };
      vi.spyOn(service, 'getFieldTestPaginationEntityIdentifier');
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.fieldTestPaginationEntitiesParams()).toMatchObject({ sort: ['id,desc'] });
  });
