...
import { asapScheduler, catchError, map, Observable, scheduled } from 'rxjs';
...
export class BankAccountMySuffixesService {
  ...
  protected readonly resourceSearchUrl = this.applicationConfigService.getEndpointFor('api/bank-accounts/_search');
...
export class BankAccountMySuffixService extends BankAccountMySuffixesService {
  ...
  search(req: Search): Observable<IBankAccountMySuffix[]> {
    const options = createRequestOption(req);
    return this.http.get<RestBankAccountMySuffix[]>(this.resourceSearchUrl, { params: options }).pipe(
