...
import { sampleWithRequiredData } from '../custom-package-parent.test-samples';
import { CustomPackageParentService } from '../service/custom-package-parent.service';

import { CustomPackageParent } from './custom-package-parent';

vi.useFakeTimers();

describe('CustomPackageParent Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: CustomPackageParent;
  let fixture: ComponentFixture<CustomPackageParent>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(CustomPackageParent);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 26792 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.customPackageParents()[0]).toEqual(expect.objectContaining({ id: 26792 }));
  });

  describe('trackId', () => {
    it('should forward to customPackageParentService', () => {
      const entity = { id: 26792 };
      vi.spyOn(service, 'getCustomPackageParentIdentifier');
      const id = comp.trackId(entity);
      expect(service.getCustomPackageParentIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.customPackageParentsParams()).toMatchObject({ sort: ['id,desc'] });
  });
