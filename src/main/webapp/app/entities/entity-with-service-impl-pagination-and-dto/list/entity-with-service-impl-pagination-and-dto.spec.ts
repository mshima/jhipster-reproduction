...
import { sampleWithRequiredData } from '../entity-with-service-impl-pagination-and-dto.test-samples';
...
describe('EntityWithServiceImplPaginationAndDTO Management Component', () => {
  ...
  let comp: EntityWithServiceImplPaginationAndDTO;
  let fixture: ComponentFixture<EntityWithServiceImplPaginationAndDTO>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(EntityWithServiceImplPaginationAndDTO);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 19834 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.entityWithServiceImplPaginationAndDTOS()[0]).toEqual(expect.objectContaining({ id: 19834 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 19834 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.entityWithServiceImplPaginationAndDTOS()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 19834 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to entityWithServiceImplPaginationAndDTOService', () => {
      const entity = { id: 19834 };
      vi.spyOn(service, 'getEntityWithServiceImplPaginationAndDTOIdentifier');
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.entityWithServiceImplPaginationAndDTOSParams()).toMatchObject({ sort: ['id,desc'] });
  });
