...
import { sampleWithRequiredData } from '../entity-with-service-class-pagination-and-dto.test-samples';
...
describe('EntityWithServiceClassPaginationAndDTO Management Component', () => {
  ...
  let comp: EntityWithServiceClassPaginationAndDTO;
  let fixture: ComponentFixture<EntityWithServiceClassPaginationAndDTO>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(EntityWithServiceClassPaginationAndDTO);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 484 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.entityWithServiceClassPaginationAndDTOS()[0]).toEqual(expect.objectContaining({ id: 484 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 484 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.entityWithServiceClassPaginationAndDTOS()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 484 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to entityWithServiceClassPaginationAndDTOService', () => {
      const entity = { id: 484 };
      vi.spyOn(service, 'getEntityWithServiceClassPaginationAndDTOIdentifier');
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.entityWithServiceClassPaginationAndDTOSParams()).toMatchObject({ sort: ['id,desc'] });
  });
