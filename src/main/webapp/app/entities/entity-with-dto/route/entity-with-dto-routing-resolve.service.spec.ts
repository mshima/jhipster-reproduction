...
import entityWithDTOResolve from './entity-with-dto-routing-resolve.service';
...
describe('EntityWithDTO routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return IEntityWithDTO returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          entityWithDTOResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(entityWithDTOResolve(mockActivatedRouteSnapshot))).rejects.toThrow('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith(123);
