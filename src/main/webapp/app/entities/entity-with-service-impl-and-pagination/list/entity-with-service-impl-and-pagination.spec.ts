...
import { sampleWithRequiredData } from '../entity-with-service-impl-and-pagination.test-samples';
...
describe('EntityWithServiceImplAndPagination Management Component', () => {
  ...
  let comp: EntityWithServiceImplAndPagination;
  let fixture: ComponentFixture<EntityWithServiceImplAndPagination>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(EntityWithServiceImplAndPagination);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 27409 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.entityWithServiceImplAndPaginations()[0]).toEqual(expect.objectContaining({ id: 27409 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 27409 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.entityWithServiceImplAndPaginations()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 27409 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to entityWithServiceImplAndPaginationService', () => {
      const entity = { id: 27409 };
      vi.spyOn(service, 'getEntityWithServiceImplAndPaginationIdentifier');
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.entityWithServiceImplAndPaginationsParams()).toMatchObject({ sort: ['id,desc'] });
  });
