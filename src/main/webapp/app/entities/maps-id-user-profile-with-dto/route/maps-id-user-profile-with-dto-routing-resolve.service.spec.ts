...
import mapsIdUserProfileWithDTOResolve from './maps-id-user-profile-with-dto-routing-resolve.service';

describe('MapsIdUserProfileWithDTO routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return IMapsIdUserProfileWithDTO returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          mapsIdUserProfileWithDTOResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(mapsIdUserProfileWithDTOResolve(mockActivatedRouteSnapshot))).rejects.toThrow('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith(123);
