...
import { sampleWithRequiredData } from '../maps-id-user-profile-with-dto.test-samples';
...
describe('MapsIdUserProfileWithDTO Management Component', () => {
  ...
  let comp: MapsIdUserProfileWithDTO;
  let fixture: ComponentFixture<MapsIdUserProfileWithDTO>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(MapsIdUserProfileWithDTO);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 8289 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.mapsIdUserProfileWithDTOS()[0]).toEqual(expect.objectContaining({ id: 8289 }));
  });

  describe('trackId', () => {
    it('should forward to mapsIdUserProfileWithDTOService', () => {
      const entity = { id: 8289 };
      vi.spyOn(service, 'getMapsIdUserProfileWithDTOIdentifier');
      const id = comp.trackId(entity);
      expect(service.getMapsIdUserProfileWithDTOIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.mapsIdUserProfileWithDTOSParams()).toMatchObject({ sort: ['id,desc'] });
  });
