...
import fieldTestEnumWithValueResolve from './field-test-enum-with-value-routing-resolve.service';

describe('FieldTestEnumWithValue routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return IFieldTestEnumWithValue returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          fieldTestEnumWithValueResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(fieldTestEnumWithValueResolve(mockActivatedRouteSnapshot))).rejects.toThrow('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith(123);
