...
import { sampleWithRequiredData } from '../field-test-service-class-and-jpa-filtering-entity.test-samples';
...
describe('FieldTestServiceClassAndJpaFilteringEntity Management Component', () => {
  ...
  let comp: FieldTestServiceClassAndJpaFilteringEntity;
  let fixture: ComponentFixture<FieldTestServiceClassAndJpaFilteringEntity>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(FieldTestServiceClassAndJpaFilteringEntity);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 26563 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.fieldTestServiceClassAndJpaFilteringEntities()[0]).toEqual(expect.objectContaining({ id: 26563 }));
  });

  describe('trackId', () => {
    it('should forward to fieldTestServiceClassAndJpaFilteringEntityService', () => {
      const entity = { id: 26563 };
      vi.spyOn(service, 'getFieldTestServiceClassAndJpaFilteringEntityIdentifier');
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.fieldTestServiceClassAndJpaFilteringEntitiesParams()).toMatchObject({ sort: ['id,desc'] });
  });
