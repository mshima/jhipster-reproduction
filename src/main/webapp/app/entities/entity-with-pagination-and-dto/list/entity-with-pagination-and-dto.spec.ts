...
import { sampleWithRequiredData } from '../entity-with-pagination-and-dto.test-samples';
...
describe('EntityWithPaginationAndDTO Management Component', () => {
  ...
  let comp: EntityWithPaginationAndDTO;
  let fixture: ComponentFixture<EntityWithPaginationAndDTO>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(EntityWithPaginationAndDTO);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 26975 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.entityWithPaginationAndDTOS()[0]).toEqual(expect.objectContaining({ id: 26975 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 26975 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.entityWithPaginationAndDTOS()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 26975 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to entityWithPaginationAndDTOService', () => {
      const entity = { id: 26975 };
      vi.spyOn(service, 'getEntityWithPaginationAndDTOIdentifier');
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.entityWithPaginationAndDTOSParams()).toMatchObject({ sort: ['id,desc'] });
  });
