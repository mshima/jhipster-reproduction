...
import fieldTestEntityResolve from './field-test-entity-routing-resolve.service';
...
describe('FieldTestEntity routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return IFieldTestEntity returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          fieldTestEntityResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(fieldTestEntityResolve(mockActivatedRouteSnapshot))).rejects.toThrow('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith(123);
