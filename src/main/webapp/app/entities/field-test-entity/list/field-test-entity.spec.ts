...
import { sampleWithRequiredData } from '../field-test-entity.test-samples';
...
describe('FieldTestEntity Management Component', () => {
  ...
  let comp: FieldTestEntity;
  let fixture: ComponentFixture<FieldTestEntity>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(FieldTestEntity);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 7565 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.fieldTestEntities()[0]).toEqual(expect.objectContaining({ id: 7565 }));
  });

  describe('trackId', () => {
    it('should forward to fieldTestEntityService', () => {
      const entity = { id: 7565 };
      vi.spyOn(service, 'getFieldTestEntityIdentifier');
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.fieldTestEntitiesParams()).toMatchObject({ sort: ['id,desc'] });
  });
