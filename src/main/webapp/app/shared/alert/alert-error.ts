...
export class AlertError implements OnDestroy {
  ...
  constructor() {
    this.errorListener = this.eventManager.subscribe('sampleWebfluxPsqlApp.error', (response: EventWithContent<unknown>) => {
      const errorResponse = (response as EventWithContent<AlertErrorModel>).content;
      this.addErrorAlert(errorResponse.message, errorResponse.key, errorResponse.params);
    });

    this.httpErrorListener = this.eventManager.subscribe('sampleWebfluxPsqlApp.httpError', (response: EventWithContent<unknown>) => {
  ...
  private handleFieldsError(httpErrorResponse: HttpErrorResponse): void {
    ...
    for (const fieldError of fieldErrors) {
      ...
      const fieldName: string = this.translateService.instant(`sampleWebfluxPsqlApp.${fieldError.objectName as string}.${convertedField}`);
