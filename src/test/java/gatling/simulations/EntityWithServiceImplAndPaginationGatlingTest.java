...
/**
 * Performance test for the EntityWithServiceImplAndPagination entity.
 *
 * @see <a href="https://github.com/jhipster/generator-jhipster/tree/v9.2.0/generators/gatling#logging-tips">Logging tips</a>
 */
public class EntityWithServiceImplAndPaginationGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all entityWithServiceImplAndPaginations")
                    .get("/api/entity-with-service-impl-and-paginations")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new entityWithServiceImplAndPagination")
                        .post("/api/entity-with-service-impl-and-paginations")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "\"hugo\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_entityWithServiceImplAndPagination_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created entityWithServiceImplAndPagination")
                            .get("${new_entityWithServiceImplAndPagination_url}")
                            .headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created entityWithServiceImplAndPagination")
                        .delete("${new_entityWithServiceImplAndPagination_url}")
                        .headers(headersHttpAuthenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the EntityWithServiceImplAndPagination entity").exec(scn);
