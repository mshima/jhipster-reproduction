...
public class CustomPackageChildGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all customPackageChildren")
                    .get("/api/custom-package-children")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new customPackageChild")
                        .post("/api/custom-package-children")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "\"childName\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_customPackageChild_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created customPackageChild").get("${new_customPackageChild_url}").headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(http("Delete created customPackageChild").delete("${new_customPackageChild_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the CustomPackageChild entity").exec(scn);
