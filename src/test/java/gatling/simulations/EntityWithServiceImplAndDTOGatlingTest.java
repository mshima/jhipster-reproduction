...
/**
 * Performance test for the EntityWithServiceImplAndDTO entity.
 *
 * @see <a href="https://github.com/jhipster/generator-jhipster/tree/v9.2.0/generators/gatling#logging-tips">Logging tips</a>
 */
public class EntityWithServiceImplAndDTOGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all entityWithServiceImplAndDTOS")
                    .get("/api/entity-with-service-impl-and-dtos")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new entityWithServiceImplAndDTO")
                        .post("/api/entity-with-service-impl-and-dtos")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "\"louis\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_entityWithServiceImplAndDTO_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created entityWithServiceImplAndDTO")
                            .get("${new_entityWithServiceImplAndDTO_url}")
                            .headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created entityWithServiceImplAndDTO")
                        .delete("${new_entityWithServiceImplAndDTO_url}")
                        .headers(headersHttpAuthenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the EntityWithServiceImplAndDTO entity").exec(scn);
