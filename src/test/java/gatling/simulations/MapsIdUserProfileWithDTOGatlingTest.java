...
/**
 * Performance test for the MapsIdUserProfileWithDTO entity.
 *
 * @see <a href="https://github.com/jhipster/generator-jhipster/tree/v9.2.0/generators/gatling#logging-tips">Logging tips</a>
 */
public class MapsIdUserProfileWithDTOGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all mapsIdUserProfileWithDTOS")
                    .get("/api/maps-id-user-profile-with-dtos")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new mapsIdUserProfileWithDTO")
                        .post("/api/maps-id-user-profile-with-dtos")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "\"dateOfBirth\": \"2020-01-01T00:00:00.000Z\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_mapsIdUserProfileWithDTO_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created mapsIdUserProfileWithDTO")
                            .get("${new_mapsIdUserProfileWithDTO_url}")
                            .headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created mapsIdUserProfileWithDTO")
                        .delete("${new_mapsIdUserProfileWithDTO_url}")
                        .headers(headersHttpAuthenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the MapsIdUserProfileWithDTO entity").exec(scn);
