...
public class CustomPackageParentGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all customPackageParents")
                    .get("/api/custom-package-parents")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new customPackageParent")
                        .post("/api/custom-package-parents")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "\"parentName\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_customPackageParent_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created customPackageParent").get("${new_customPackageParent_url}").headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(http("Delete created customPackageParent").delete("${new_customPackageParent_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the CustomPackageParent entity").exec(scn);
