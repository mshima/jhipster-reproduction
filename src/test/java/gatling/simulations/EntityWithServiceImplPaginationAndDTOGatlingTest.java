...
/**
 * Performance test for the EntityWithServiceImplPaginationAndDTO entity.
 *
 * @see <a href="https://github.com/jhipster/generator-jhipster/tree/v9.2.0/generators/gatling#logging-tips">Logging tips</a>
 */
public class EntityWithServiceImplPaginationAndDTOGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all entityWithServiceImplPaginationAndDTOS")
                    .get("/api/entity-with-service-impl-pagination-and-dtos")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new entityWithServiceImplPaginationAndDTO")
                        .post("/api/entity-with-service-impl-pagination-and-dtos")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "\"theo\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_entityWithServiceImplPaginationAndDTO_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created entityWithServiceImplPaginationAndDTO")
                            .get("${new_entityWithServiceImplPaginationAndDTO_url}")
                            .headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created entityWithServiceImplPaginationAndDTO")
                        .delete("${new_entityWithServiceImplPaginationAndDTO_url}")
                        .headers(headersHttpAuthenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the EntityWithServiceImplPaginationAndDTO entity").exec(scn);
