...
/**
 * Performance test for the EntityWithPaginationAndDTO entity.
 *
 * @see <a href="https://github.com/jhipster/generator-jhipster/tree/v9.2.0/generators/gatling#logging-tips">Logging tips</a>
 */
public class EntityWithPaginationAndDTOGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all entityWithPaginationAndDTOS")
                    .get("/api/entity-with-pagination-and-dtos")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new entityWithPaginationAndDTO")
                        .post("/api/entity-with-pagination-and-dtos")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "\"lea\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_entityWithPaginationAndDTO_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created entityWithPaginationAndDTO")
                            .get("${new_entityWithPaginationAndDTO_url}")
                            .headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created entityWithPaginationAndDTO")
                        .delete("${new_entityWithPaginationAndDTO_url}")
                        .headers(headersHttpAuthenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the EntityWithPaginationAndDTO entity").exec(scn);
