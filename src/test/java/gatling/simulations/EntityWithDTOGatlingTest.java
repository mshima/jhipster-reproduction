...
public class EntityWithDTOGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all entityWithDTOS").get("/api/entity-with-dtos").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new entityWithDTO")
                        .post("/api/entity-with-dtos")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "\"emma\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_entityWithDTO_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created entityWithDTO").get("${new_entityWithDTO_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created entityWithDTO").delete("${new_entityWithDTO_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the EntityWithDTO entity").exec(scn);
