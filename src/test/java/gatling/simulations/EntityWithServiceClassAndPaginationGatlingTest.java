...
/**
 * Performance test for the EntityWithServiceClassAndPagination entity.
 *
 * @see <a href="https://github.com/jhipster/generator-jhipster/tree/v9.2.0/generators/gatling#logging-tips">Logging tips</a>
 */
public class EntityWithServiceClassAndPaginationGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all entityWithServiceClassAndPaginations")
                    .get("/api/entity-with-service-class-and-paginations")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new entityWithServiceClassAndPagination")
                        .post("/api/entity-with-service-class-and-paginations")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "\"enzo\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_entityWithServiceClassAndPagination_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created entityWithServiceClassAndPagination")
                            .get("${new_entityWithServiceClassAndPagination_url}")
                            .headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created entityWithServiceClassAndPagination")
                        .delete("${new_entityWithServiceClassAndPagination_url}")
                        .headers(headersHttpAuthenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the EntityWithServiceClassAndPagination entity").exec(scn);
