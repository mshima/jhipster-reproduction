...
/**
 * Performance test for the EntityWithServiceClassPaginationAndDTO entity.
 *
 * @see <a href="https://github.com/jhipster/generator-jhipster/tree/v9.2.0/generators/gatling#logging-tips">Logging tips</a>
 */
public class EntityWithServiceClassPaginationAndDTOGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all entityWithServiceClassPaginationAndDTOS")
                    .get("/api/entity-with-service-class-pagination-and-dtos")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new entityWithServiceClassPaginationAndDTO")
                        .post("/api/entity-with-service-class-pagination-and-dtos")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "\"lena\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_entityWithServiceClassPaginationAndDTO_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created entityWithServiceClassPaginationAndDTO")
                            .get("${new_entityWithServiceClassPaginationAndDTO_url}")
                            .headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created entityWithServiceClassPaginationAndDTO")
                        .delete("${new_entityWithServiceClassPaginationAndDTO_url}")
                        .headers(headersHttpAuthenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the EntityWithServiceClassPaginationAndDTO entity").exec(scn);
