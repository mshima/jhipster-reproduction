...
describe('BankAccount e2e test', () => {
  ...
  let username: string;
  let password: string;
  const bankAccountSample = { name: 'account exhaust', balance: 22278.86 };
  ...
  describe('BankAccount page', () => {
    it('should have translated page title', () => {
      ...
      cy.getEntityHeading('BankAccount').should('not.contain', 'sampleWebfluxH2MemApp.testRootBankAccount.home.title');
    });
  ...
  describe('new BankAccount page', () => {
    ...
    it('should create an instance of BankAccount', () => {
      cy.get(`[data-cy="name"]`).type('revere shore tooth');
      cy.get(`[data-cy="name"]`).should('have.value', 'revere shore tooth');

      cy.get(`[data-cy="guid"]`).type('f45f3d68-b349-4385-96b3-cce52f3c66d5');
      cy.get(`[data-cy="guid"]`).invoke('val').should('match', new RegExp('f45f3d68-b349-4385-96b3-cce52f3c66d5'));

      cy.get(`[data-cy="bankNumber"]`).type('23108');
      cy.get(`[data-cy="bankNumber"]`).should('have.value', '23108');

      cy.get(`[data-cy="agencyNumber"]`).type('19586');
      cy.get(`[data-cy="agencyNumber"]`).should('have.value', '19586');

      cy.get(`[data-cy="lastOperationDuration"]`).type('8870.73');
      cy.get(`[data-cy="lastOperationDuration"]`).should('have.value', '8870.73');

      cy.get(`[data-cy="meanOperationDuration"]`).type('4779.55');
      cy.get(`[data-cy="meanOperationDuration"]`).should('have.value', '4779.55');

      cy.get(`[data-cy="meanQueueDuration"]`).type('PT47M');
      cy.get(`[data-cy="meanQueueDuration"]`).blur();
      cy.get(`[data-cy="meanQueueDuration"]`).should('have.value', 'PT47M');

      cy.get(`[data-cy="balance"]`).type('18636.14');
      cy.get(`[data-cy="balance"]`).should('have.value', '18636.14');

      cy.get(`[data-cy="openingDay"]`).type('2020-08-03');
      cy.get(`[data-cy="openingDay"]`).blur();
      cy.get(`[data-cy="openingDay"]`).should('have.value', '2020-08-03');

      cy.get(`[data-cy="lastOperationDate"]`).type('2020-08-04T03:35');
      cy.get(`[data-cy="lastOperationDate"]`).blur();
      cy.get(`[data-cy="lastOperationDate"]`).should('have.value', '2020-08-04T03:35');
