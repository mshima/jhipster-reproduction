...
describe('UserManagement e2e test', () => {
  ...
  let username: string;
  let password: string;
  const userManagementSample = { login: 'Jessica.Bergnaum', email: 'Kelly_Haag@hotmail.com' };
  ...
  describe('new UserManagement page', () => {
    ...
    it('should create an instance of UserManagement', () => {
      cy.get(`[data-cy="login"]`).type('Elouise_OReilly');
      cy.get(`[data-cy="login"]`).should('have.value', 'Elouise_OReilly');

      cy.get(`[data-cy="firstName"]`).type('Dulce');
      cy.get(`[data-cy="firstName"]`).should('have.value', 'Dulce');

      cy.get(`[data-cy="lastName"]`).type('Durgan');
      cy.get(`[data-cy="lastName"]`).should('have.value', 'Durgan');

      cy.get(`[data-cy="email"]`).type('Broderick92@hotmail.com');
      cy.get(`[data-cy="email"]`).should('have.value', 'Broderick92@hotmail.com');
