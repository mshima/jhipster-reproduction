...
describe('UserManagement e2e test', () => {
  ...
  let username: string;
  let password: string;
  const userManagementSample = { login: 'Adan29', email: 'Amani_Sanford@gmail.com' };
  ...
  describe('new UserManagement page', () => {
    ...
    it('should create an instance of UserManagement', () => {
      cy.get(`[data-cy="login"]`).type('Marian.Stoltenberg');
      cy.get(`[data-cy="login"]`).should('have.value', 'Marian.Stoltenberg');

      cy.get(`[data-cy="firstName"]`).type('Mya');
      cy.get(`[data-cy="firstName"]`).should('have.value', 'Mya');

      cy.get(`[data-cy="lastName"]`).type('Hilll');
      cy.get(`[data-cy="lastName"]`).should('have.value', 'Hilll');

      cy.get(`[data-cy="email"]`).type('Torrance.Kuhlman94@hotmail.com');
      cy.get(`[data-cy="email"]`).should('have.value', 'Torrance.Kuhlman94@hotmail.com');
