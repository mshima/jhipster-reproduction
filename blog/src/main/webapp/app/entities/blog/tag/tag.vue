<template>
  <div>
    <h2 id="page-heading" data-cy="TagHeading">
      <span id="tag">{{ t$('blogApp.blogTag.home.title') }}</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info me-2" @click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span>{{ t$('blogApp.blogTag.home.refreshListLabel') }}</span>
        </button>
        <router-link :to="{ name: 'TagCreate' }" custom v-slot="{ navigate }">
          <button @click="navigate" id="jh-create-entity" data-cy="entityCreateButton" class="btn btn-primary jh-create-entity create-tag">
            ...
            <span>{{ t$('blogApp.blogTag.home.createLabel') }}</span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && tags?.length === 0">
      <span>{{ t$('blogApp.blogTag.home.notFound') }}</span>
    </div>
        ...
        <thead>
          <tr>
            ...
            <th scope="col" @click="changeOrder('name')">
              <span>{{ t$('blogApp.blogTag.name') }}</span>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <template #title>
        <span id="blogApp.blogTag.delete.question" data-cy="tagDeleteDialogHeading">{{ t$('entity.delete.title') }}</span>
      </template>
      <div class="modal-body">
        <p id="jhi-delete-tag-heading">{{ t$('blogApp.blogTag.delete.question', { id: removeId }) }}</p>
      </div>
      <template #footer>
        <div>
