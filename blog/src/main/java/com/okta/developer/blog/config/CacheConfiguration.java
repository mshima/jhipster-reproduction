...
public class CacheConfiguration {
    ...
    public InfinispanGlobalConfigurer globalConfiguration(JHipsterProperties jHipsterProperties) {
        ...
        if (this.registration == null) {
            // if registry is not defined, use native discovery
            ...
            return () ->
                GlobalConfigurationBuilder.defaultClusteredBuilder()
                    ...
                    .clusterName("infinispan-blog-cluster")
                    .jmx()
                    .enabled(jHipsterProperties.getCache().getInfinispan().isStatsEnabled())
                    .serialization()
                    .marshaller(new JBossUserMarshaller())
                    .build();
        } else if (discoveryClient.getInstances(registration.getServiceId()).isEmpty()) {
            return () ->
                GlobalConfigurationBuilder.defaultClusteredBuilder()
                    .transport()
    ...
    public InfinispanCacheConfigurer cacheConfigurer(JHipsterProperties jHipsterProperties) {
        ...
        return manager -> {
            ...
            Stream.of(
                com.okta.developer.blog.repository.UserRepository.USERS_BY_LOGIN_CACHE,
                com.okta.developer.blog.repository.UserRepository.USERS_BY_EMAIL_CACHE
            ).forEach(cacheName ->
                manager.defineConfiguration(
                    cacheName,
                    new ConfigurationBuilder()
