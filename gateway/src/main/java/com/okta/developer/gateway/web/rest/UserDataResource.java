...
public class UserDataResource {
    ...
    public Mono<ResponseEntity<UserData>> createUserData(@RequestBody UserData userData) throws URISyntaxException {
        ...
        return userDataRepository.save(userData).map(result -> {
            try {
    ...
    public Mono<ResponseEntity<UserData>> updateUserData(
        ...
        return userDataRepository.existsById(id).flatMap(exists -> {
            if (!exists) {
        ...
        return userDataRepository.existsById(id).flatMap(exists -> {
            ...
            Mono<UserData> result = userDataRepository
                ...
                .map(existingUserData -> {
                    updateIfPresent(existingUserData::setAddress, userData.getAddress());
    ...
    /**
     * {@code GET  /user-data} : get all the User Data.
     *
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of User Data in body.
     */
    @GetMapping(value = "", produces = MediaType.APPLICATION_JSON_VALUE)
    public Mono<List<UserData>> getAllUserDatas() {
        LOG.debug("REST request to get all UserDatas");
        return userDataRepository.findAll().collectList();
    }

    /**
     * {@code GET  /user-data} : get all the User Data as a stream.
     * @return the {@link Flux} of User Data.
     */
    @GetMapping(value = "", produces = MediaType.APPLICATION_NDJSON_VALUE)
    public Flux<UserData> getAllUserDatasAsStream() {
        LOG.debug("REST request to get all UserDatas as a stream");
