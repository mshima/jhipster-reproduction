...
export const lazyRoutes = Promise.all([
  loadRemote<any>('gateway/entities-router')
    .then(gatewayRouter => {
      router.addRoute(gatewayRouter.default as RouteRecordRaw);
      return gatewayRouter.default;
    })
    .catch(error => {
      console.log(`Error loading gateway menus. Make sure it's up. ${error}`);
    }),
  ...
  loadRemote<any>('notification/entities-router')
    .then(notificationRouter => {
      router.addRoute(notificationRouter.default as RouteRecordRaw);
      return notificationRouter.default;
    })
    .catch(error => {
      console.log(`Error loading notification menus. Make sure it's up. ${error}`);
    }),
]);
