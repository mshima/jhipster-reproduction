    ...
    <div class="col-8">
      <div v-if="product">
        <h2 class="jh-entity-heading" data-cy="productDetailsHeading">
          <span>{{ t$('gatewayApp.storeProduct.detail.title') }}</span> {{ product.id }}
        </h2>
        <dl class="row-md jh-entity-details">
          <dt>
            <span>{{ t$('gatewayApp.storeProduct.title') }}</span>
          </dt>
          <dd>
            <span>{{ product.title }}</span>
          </dd>
          <dt>
            <span>{{ t$('gatewayApp.storeProduct.price') }}</span>
          </dt>
          <dd>
            <span>{{ product.price }}</span>
          </dd>
          <dt>
            <span>{{ t$('gatewayApp.storeProduct.image') }}</span>
          </dt>
          <dd>
