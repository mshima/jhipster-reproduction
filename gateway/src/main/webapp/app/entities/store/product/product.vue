<template>
  <div>
    <h2 id="page-heading" data-cy="ProductHeading">
      <span id="product">{{ t$('gatewayApp.storeProduct.home.title') }}</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info me-2" @click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span>{{ t$('gatewayApp.storeProduct.home.refreshListLabel') }}</span>
        </button>
        <router-link :to="{ name: 'ProductCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span>{{ t$('gatewayApp.storeProduct.home.createLabel') }}</span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && products?.length === 0">
      <span>{{ t$('gatewayApp.storeProduct.home.notFound') }}</span>
    </div>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <template #title>
        <span id="gatewayApp.storeProduct.delete.question" data-cy="productDeleteDialogHeading">{{ t$('entity.delete.title') }}</span>
      </template>
      <div class="modal-body">
        <p id="jhi-delete-product-heading">{{ t$('gatewayApp.storeProduct.delete.question', { id: removeId }) }}</p>
      </div>
      <template #footer>
        <div>
